...
private:
    ...
    void extractPkgFile(const QString& pkgPath, const QString& outputPath) {
        // Determine PKG type from the path using downloads_folder logic
        ...
        switch (pkgType) {
            case PkgType::BaseGame:
                extractBaseGame(pkgPath, outputPath);
                break;
            case PkgType::Update:
                extractUpdate(pkgPath, outputPath);
                break;
            case PkgType::DLC:
                extractDLC(pkgPath, outputPath);
                break;
            default:
                QMessageBox::warning(this, "Unknown PKG Type", 
                    QString("Could not determine PKG type for: %1\nExtracting as base game.").arg(baseName));
                extractBaseGame(pkgPath, outputPath);
                break;
        }
    }

    // Common validation used by all extraction types
    bool validatePkgFile(const QString& pkgPath) {
    ...
    void extractBaseGame(const QString& pkgPath, const QString& outputPath) {
        if (!validatePkgFile(pkgPath)) return;
        
        try {
            performStandardExtraction(pkgPath, outputPath, "Extracting Base Game");
        } catch (const std::exception& e) {
            QMessageBox::critical(this, "Base Game Extraction Failed", 
                QString("Error extracting base game: %1").arg(QString::fromStdString(e.what())));
        }
    }

    void extractUpdate(const QString& pkgPath, const QString& outputPath) {
        if (!validatePkgFile(pkgPath)) return;
        
        try {
            // Updates extract directly to their folder (NO MERGING!)
            performStandardExtraction(pkgPath, outputPath, "Extracting Update");
        } catch (const std::exception& e) {
            QMessageBox::critical(this, "Update Extraction Failed", 
                QString("Error extracting update: %1").arg(QString::fromStdString(e.what())));
        }
    }

    void extractDLC(const QString& pkgPath, const QString& outputPath) {
        if (!validatePkgFile(pkgPath)) return;
        
        try {
            performStandardExtraction(pkgPath, outputPath, "Extracting DLC");
        } catch (const std::exception& e) {
            QMessageBox::critical(this, "DLC Extraction Failed", 
                QString("Error extracting DLC: %1").arg(QString::fromStdString(e.what())));
        }
    }

    // Common extraction logic used by all PKG types
    void performStandardExtraction(const QString& pkgPath, const QString& outputPath, const QString& progressTitle) {
        ...
        // Display metadata information during extraction
        QMessageBox::information(this, progressTitle, 
            QString("PKG Metadata:\nTitle ID: %1\nContent Type: %2\nPKG Type: %3\nSize: %4 MB\nFiles: %5")
            .arg(QString::fromStdString(metadata.title_id))
            .arg(metadata.content_type)
            .arg(metadata.pkg_type)
            .arg(metadata.pkg_size / (1024 * 1024))
            .arg(metadata.file_count));

        // Setup progress dialog
        QProgressDialog* progressDialog = new QProgressDialog(progressTitle, "Cancel", 0, 100, this);
        ...
        connect(progressDialog, &QProgressDialog::canceled, this, [progressDialog]() {
            QMessageBox::information(progressDialog, "Canceled", "Extraction was cancelled by user.");
        });

        // Create output directory
        std::error_code ec;
        std::filesystem::create_directories(outputPathFs, ec);
        if (ec) {
            progressDialog->close();
            delete progressDialog;
            QMessageBox::critical(this, "Extraction Error", 
                QString("Failed to create output directory: %1").arg(outputPath));
            return;
        }

        // Use a local variable that can be captured by the lambda
        QProgressDialog* localProgress = progressDialog;
        
        // Set up progress callback
        pkg.SetProgressCallback([localProgress](const PKGProgress& progress) {
            if (localProgress && !localProgress->wasCanceled()) {
                localProgress->setValue(static_cast<int>(progress.percent));
                QApplication::processEvents();
            }
        });
        
        // Perform the extraction directly to the target folder
        bool success = pkg.Extract(pkgPathFs, outputPathFs, failReason);
        ...
        if (success) {
            QMessageBox::information(this, "Success", 
                QString("PKG extraction completed successfully!\n\nExtracted to: %1").arg(outputPath));
        } else {
            QMessageBox::critical(this, "Extraction Error", 
                QString("PKG extraction failed: %1\n\nTarget: %2").arg(QString::fromStdString(failReason)).arg(outputPath));
        }
    }
    ...
    PkgType detectPkgType(const QString& baseName) {
        ...
        if (lowerBaseName.contains("update") || 
            lowerBaseName.contains("-u") ||
            lowerBaseName.contains("_u") ||
            lowerBaseName.contains("patch")) {
            return PkgType::Update;
        }
        
        // Check for DLC pattern
